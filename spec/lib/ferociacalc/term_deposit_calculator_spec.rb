  ...
  describe '.inputs' do
    ...
    describe 'initial_deposit' do
      ...
      it 'requires positive values gte 1_000' do
        expect { input[:validator].call(1_000) }.to_not raise_error

      end

      it 'raises on values lt 1_000' do
        expect { input[:validator].call(10) }.to raise_error(/must provide a valid initial deposit amount/)
      end

      it 'raises on values gt 1_500_000' do
        expect { input[:validator].call(2_000_000) }.to raise_error(/must provide a valid initial deposit amount/)
      end

      it 'has a noop marshal' do
        expect(input[:transformer].call(1_000)).to eq(1_000)
      end
    end
    ...
    describe 'interest_rate' do
      ...
      it 'requires positive values' do
        expect { input[:validator].call(2) }.to_not raise_error

      end

      it 'raises on negative values' do
        expect { input[:validator].call(-2) }.to raise_error(/must provide a valid interest rate number/)
      end

      it 'is marshalled into a decimal' do
        expect(input[:transformer].call(3.5)).to eq(0.035)
      end
    end
    ...
    describe 'deposit_term' do
      ...
      it 'requires a valid value' do
        expect { input[:validator].call(3) }.not_to raise_error

      end

      it 'raises on negative values' do
        expect { input[:validator].call(-2) }.to raise_error(/must provide a valid number of months/)
      end

      it 'is marshalled from months into years' do
        expect(input[:transformer].call(18)).to eq(1.5)
      end
    end
    ...
    describe 'interest_period' do
      ...
      it 'requires a valid interest period' do
        expect { input[:validator].call('monthly') }.to_not raise_error

      end

      it 'raises on an invalid interest period' do
        expect { input[:validator].call('daily') }.to raise_error(/must provide a valid interest period/)
      end
    end
  end
end
